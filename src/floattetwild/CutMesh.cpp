...
void floatTetWild::print_times1()
{
    ...
    logger().debug("\t\t- time_get_intersecting_edges_and_points1 = {}s",
                   time_get_intersecting_edges_and_points1);
    logger().debug("\t\t- time_get_intersecting_edges_and_points2 = {}s",
                   time_get_intersecting_edges_and_points2);
    logger().debug("\t\t- time_get_intersecting_edges_and_points3 = {}s",
                   time_get_intersecting_edges_and_points3);
    logger().debug("\t\t- time_get_intersecting_edges_and_points4 = {}s",
                   time_get_intersecting_edges_and_points4);
}
...
void floatTetWild::CutMesh::construct(const std::vector<int>& cut_t_ids)
{
    ...
    //
    //    for (int i = 0; i < tets.size(); i++) {//todo: construct conn_tets/opp_t_ids only when
    //    expension is required.
    //        for (int j = 0; j < 4; j++) {
    //            if (opp_t_ids[i][j] >= 0)
    //                continue;
    //
    ...
    //            set_intersection_sorted(conn_tets[tets[i][(j + 1) % 4]], conn_tets[tets[i][(j + 2)
    //            % 4]],
    ...
    //            for (int k = 0; k < 4; k++) {
    //                if (tets[n_t_id][k] != tets[i][(j + 1) % 4] && tets[n_t_id][k] != tets[i][(j +
    //                2) % 4]
...
void floatTetWild::CutMesh::expand(std::vector<int>& cut_t_ids)
{
    ...
    for (int i = 0; i < tets.size(); i++) {
        for (int j = 0; j < 4; j++) {
            ...
            std::vector<int> n_t_ids;
            set_intersection_sorted(conn_tets[tets[i][(j + 1) % 4]],
                                    conn_tets[tets[i][(j + 2) % 4]],
                                    conn_tets[tets[i][(j + 3) % 4]],
                                    n_t_ids);
            ...
            opp_t_ids[i][j] = n_t_id;
            for (int k = 0; k < 4; k++) {
                if (tets[n_t_id][k] != tets[i][(j + 1) % 4] &&
                    tets[n_t_id][k] != tets[i][(j + 2) % 4] &&
                    tets[n_t_id][k] != tets[i][(j + 3) % 4]) {
                    opp_t_ids[n_t_id][k] = i;
                    break;
                }
        ...
        for (int i = 0; i < new_opp_t_ids.size(); i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                if (map_v_ids.find(v_id) == map_v_ids.end()) {
                    ...
                    int    ori =
                      Predicates::orient_3d(p_vs[0], p_vs[1], p_vs[2], mesh.tet_vertices[v_id].pos);
                    if ((ori == Predicates::ORI_NEGATIVE &&
                         dist < 0)  // todo: change get_to_plane_dist return value sign
                    ...
                    if (ori != Predicates::ORI_ZERO &&
                        std::fabs(to_plane_dists[lv_id]) < mesh.params.eps_coplanar)
            ...
            opp_t_ids.push_back({{new_opp_t_ids[i][0],
                                  new_opp_t_ids[i][1],
                                  new_opp_t_ids[i][2],
                                  new_opp_t_ids[i][3]}});
            for (int j = 0; j < 4; j++) {
                if (opp_t_ids.back()[j] < 0) {
                    if (t[(j + 1) % 4] < old_v_ids_size && t[(j + 2) % 4] < old_v_ids_size &&
                        ...
                        std::vector<int> tmp;
                        set_intersection(conn_tets[t[(j + 1) % 4]],
                                         conn_tets[t[(j + 2) % 4]],
                                         conn_tets[t[(j + 3) % 4]],
                                         tmp);
                        if (
                          tmp.size() ==
                          1)  // 一个三角形刚好填了一个v字的缺口，所有点都在v_ids中，但三角形不在cut_mesh中
                            continue;
                ...
                for (int k = 0; k < 4; k++) {
                    if (tets[opp_t_id][k] != t[(j + 1) % 4] &&
                        tets[opp_t_id][k] != t[(j + 2) % 4] &&
...
void floatTetWild::CutMesh::expand_new(std::vector<int>& cut_t_ids)
{
    ...
    const std::array<Vector2, 3> tri_2d = {
      {to_2d(p_vs[0], t), to_2d(p_vs[1], t), to_2d(p_vs[2], t)}};
    ...
    while (true) {
        ...
        for (auto m : map_v_ids) {
            ...
            for (int gt_id : mesh.tet_vertices[gv_id].conn_tets) {
                ...
                for (int j = 0; j < 4; j++) {
                    int ori = Predicates::orient_3d(
                      p_vs[0], p_vs[1], p_vs[2], mesh.tet_vertices[mesh.tets[gt_id][j]].pos);
                    if (ori == Predicates::ORI_POSITIVE)
                        cnt_pos++;
                    else if (ori == Predicates::ORI_NEGATIVE)
                        cnt_neg++;
                ...
                for (int j = 0; j < 4; j++) {
                    if (is_tri_tri_cutted_2d(
                          {{tet_2d[(j + 1) % 4], tet_2d[(j + 2) % 4], tet_2d[(j + 3) % 4]}},
                          tri_2d)) {
                        is_overlapped = true;
                        break;
                    }
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    if (map_v_ids.find(new_gv_id) == map_v_ids.end()) {
                        ...
                        int    ori  = Predicates::orient_3d(
                          p_vs[0], p_vs[1], p_vs[2], mesh.tet_vertices[new_gv_id].pos);
    ...
    //                tmp_cnt++;
    //                cout << i << " gt_id " << cut_t_ids[i] << " should include the neighbor!!!" <<
    //                endl; cout << "opp_t_id = " << get_opp_t_id(cut_t_ids[i], j, mesh) << endl;
  ...
  std::vector<int>&                  subdivide_t_ids)
{
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        Vector3 p;
        Scalar  _;
        bool    is_result = seg_plane_intersection(
          mesh.tet_vertices[v1_id].pos, mesh.tet_vertices[v2_id].pos, p_vs[0], p_n, p, _);
        ...
        //        mesh.tet_vertices[v2_id].conn_tets, tmp);
    ...
    for (int v_id : e_v_ids)
        subdivide_t_ids.insert(subdivide_t_ids.end(),
...
void floatTetWild::CutMesh::get_one_ring_t_ids(std::vector<int>& old_t_ids,
                                               std::vector<int>& neighbor_t_ids)
{
    ...
    //        tmp_t_ids.insert(neighbor_t_ids.end(),
    //        mesh.tet_vertices[v_ids[lv_id]].conn_tets.begin(),
    //                         mesh.tet_vertices[v_ids[lv_id]].conn_tets.end());
    //    }
    //    vector_unique(tmp_t_ids);
    //
    //    std::set_difference(tmp_t_ids.begin(), tmp_t_ids.end(), old_t_ids.begin(),
    //    old_t_ids.end(),
...
bool floatTetWild::CutMesh::check()
{
    ...
    for (auto& m : map_v_ids) {
        ...
        if (std::fabs(dist) < mesh.params.eps_coplanar && to_plane_dists[lv_id] != 0 &&
            !is_snapped[lv_id]) {
