...
namespace {
void get_bb_corners(const Parameters&           params,
                    ...
                    Vector3&                    max)
{
    ...
    //            const Scalar dis = std::max((max - min).minCoeff() * params.box_scale,
    //            params.eps_input * 2);
...
void match_surface_fs(Mesh&                        mesh,
...
void compute_voxel_points(const Vector3&        min,
                          const Vector3&        max,
...
// #include <floattetwild/external/Predicates.hpp>
//     extern "C" floatTetWild::Scalar orient3d(const floatTetWild::Scalar *pa, const
//     floatTetWild::Scalar *pb, const floatTetWild::Scalar *pc, const floatTetWild::Scalar *pd);

void FloatTetDelaunay::tetrahedralize(const std::vector<Vector3>&  input_vertices,
                                      const std::vector<Vector3i>& input_faces,
                                      const AABBWrapper&           tree,
                                      Mesh&                        mesh,
                                      std::vector<bool>&           is_face_inserted)
{
    ...
    for (int i = 0; i < mesh.tets.size(); i++) {
        auto& t = mesh.tets[i];
        if (is_inverted(mesh.tet_vertices[t[0]].pos,
