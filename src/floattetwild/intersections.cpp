...
bool floatTetWild::seg_line_intersection_2d(const std::array<Vector2, 2>& seg,
                                            const std::array<Vector2, 2>& line,
                                            Scalar&                       t_seg)
{
    // assumptions:
...
bool floatTetWild::seg_seg_intersection_2d(const std::array<Vector2, 2>& seg1,
                                           const std::array<Vector2, 2>& seg2,
                                           Scalar&                       t2)
{
    // assumptions:
    // segs are not degenerate
...
floatTetWild::Scalar floatTetWild::seg_seg_squared_dist_3d(const std::array<Vector3, 2>& s1,
                                                           const std::array<Vector3, 2>& s2)
{
...
floatTetWild::Scalar floatTetWild::p_line_squared_dist_3d(const Vector3& v,
                                                          const Vector3& a,
                                                          const Vector3& b)
{
...
floatTetWild::Scalar floatTetWild::p_seg_squared_dist_3d(const Vector3& v,
                                                         const Vector3& a,
                                                         const Vector3& b)
{
    Vector3 av = v - a;
...
bool floatTetWild::seg_plane_intersection(const Vector3& p1,
                                          const Vector3& p2,
                                          const Vector3& a,
                                          const Vector3& n,
                                          Vector3&       p,
...
int floatTetWild::is_tri_tri_cutted(const std::array<Vector3, 3>& f_tri,
                                    const std::array<Vector3, 3>& f_tet,
                                    const std::array<int, 3>&     oris_tri)
{
    ...
    if (cnt_zero == 2) {
        ...
        for (int j = 0; j < 3; j++) {
            if (oris_tri[j] == oris_tri[(j + 1) % 3] &&
                is_seg_tri_cutted_2d(
                  {{to_2d(f_tet[j], t), to_2d(f_tet[(j + 1) % 3], t)}},
    ...
    auto is_f1_cut_f2 = [](const std::array<Vector3, 3>& f1,
                           const std::array<Vector3, 3>& f2,
                           const std::array<int, 3>&     oris) {  // check f2 plane
        Vector3 n = ((f2[1] - f2[2]).cross(f2[0] - f2[2])).normalized();
        int     t = get_t(f2[0], f2[1], f2[2]);
        for (int j = 0; j < 3; j++) {
            if (oris[j] == Predicates::ORI_NEGATIVE &&
                  oris[(j + 1) % 3] == Predicates::ORI_POSITIVE ||
                oris[(j + 1) % 3] == Predicates::ORI_NEGATIVE &&
                  oris[j] == Predicates::ORI_POSITIVE) {
                Vector3 p;
                Scalar  _;
                if (seg_plane_intersection(f1[j], f1[(j + 1) % 3], n, f2[0], p, _) &&
                    is_p_inside_tri_2d(to_2d(p, t),
                                       {{to_2d(f2[0], t), to_2d(f2[1], t), to_2d(f2[2], t)}}))
                    return true;
            }
        }
        return false;
...
bool floatTetWild::is_tri_tri_cutted_2d(const std::array<Vector2, 3>& vs_tet,
                                        const std::array<Vector2, 3>& vs_tri)
{
    ...
    if (cnt_neg0 == 3 || cnt_pos0 == 3 || cnt_neg1 == 3 || cnt_pos1 == 3 || cnt_neg2 == 3 ||
        cnt_pos2 == 3)
    ...
    for (int tri_e_id = 0; tri_e_id < 3; tri_e_id++) {
        for (int tet_e_id = 0; tet_e_id < 3; tet_e_id++) {
            if (is_crossing(tri_tet[tri_e_id * 3 + tet_e_id],
                            tri_tet[tri_e_id * 3 + (tet_e_id + 1) % 3]) &&
                is_crossing(tet_tri[tet_e_id * 3 + tri_e_id],
                            tet_tri[tet_e_id * 3 + (tri_e_id + 1) % 3]))
                return true;
        }
    }
...
bool floatTetWild::is_seg_tri_cutted_2d(const std::array<Vector2, 2>& seg,
                                        const std::array<Vector2, 3>& tri)
{
    ...
    if (cnt_neg0 == 3 || cnt_pos0 == 3 || cnt_neg1 == 3 ||
        cnt_pos1 == 3)  // one of the endpoints is contained inside tri
        return true;
...
floatTetWild::Vector2 floatTetWild::to_2d(const Vector3& p,
                                          const Vector3& n,
                                          const Vector3& pp,
                                          int            t)
{
...
extern "C++" int tri_tri_intersection_test_3d(floatTetWild::Scalar p1[3],
                                              floatTetWild::Scalar q1[3],
                                              floatTetWild::Scalar r1[3],
                                              floatTetWild::Scalar p2[3],
                                              floatTetWild::Scalar q2[3],
                                              floatTetWild::Scalar r2[3],
                                              int*                 coplanar,
                                              floatTetWild::Scalar source[3],
                                              floatTetWild::Scalar target[3]);

int floatTetWild::is_tri_tri_cutted(const Vector3& p1,
                                    const Vector3& p2,
                                    const Vector3& p3,
                                    const Vector3& q1,
                                    const Vector3& q2,
                                    const Vector3& q3)
{
    ...
    if (!tri_tri_intersection_test_3d(
          &p_1[0], &q_1[0], &r_1[0], &p_2[0], &q_2[0], &r_2[0], &coplanar, &s[0], &t[0]))
        return CUT_EMPTY;
...
int floatTetWild::is_tri_tri_cutted_hint(const Vector3& p1,
                                         const Vector3& p2,
                                         const Vector3& p3,
                                         const Vector3& q1,
                                         const Vector3& q2,
                                         ...
                                         bool           is_debug)
{
    ...
    if (hint == CUT_COPLANAR) {
        ...
        if (is_tri_tri_cutted_2d({{to_2d(p1, t), to_2d(p2, t), to_2d(p3, t)}},
                                 {{to_2d(q1, t), to_2d(q2, t), to_2d(q3, t)}}))
            return CUT_COPLANAR;
        return CUT_EMPTY;
    }
    ...
    if (is_debug) {
        ...
        //
        //        cout<<"res = "<<tri_tri_intersection_test_3d(&p_1[0], &q_1[0], &r_1[0], &p_2[0],
        //        &q_2[0], &r_2[0], &coplanar, &s[0], &t[0])<<endl; cout<<std::setprecision(17);
        //        cout<<"s"<<endl;
    ...
    int result = tri_tri_intersection_test_3d(
      &p_1[0], &q_1[0], &r_1[0], &p_2[0], &q_2[0], &r_2[0], &coplanar, &s[0], &t[0]);
    if (is_debug) {
    ...
    if (std::abs(s[0] - t[0]) <= SCALAR_ZERO && std::abs(s[1] - t[1]) <= SCALAR_ZERO &&
        std::abs(s[2] - t[2]) <= SCALAR_ZERO)
        return CUT_EMPTY;
...
void floatTetWild::get_bbox_face(const Vector3& p0,
                                 const Vector3& p1,
                                 const Vector3& p2,
                                 Vector3&       min,
                                 Vector3&       max,
...
void floatTetWild::get_bbox_tet(const Vector3& p0,
                                const Vector3& p1,
                                const Vector3& p2,
                                const Vector3& p3,
                                Vector3&       min,
...
bool floatTetWild::is_bbox_intersected(const Vector3& min1,
                                       const Vector3& max1,
                                       const Vector3& min2,
                                       const Vector3& max2)
{
...
bool floatTetWild::is_point_inside_tet(const Vector3& p,
                                       const Vector3& p0t,
                                       const Vector3& p1t,
                                       const Vector3& p2t,
                                       const Vector3& p3t)
